...
class recuitAgent(mesa.Agent):
    def __init__(self, id, new_util, t, max_iter, a, model):
        ...
        self.model = model
        self.new_cycle = True
        self.s_best = util.generate_initial_solution(self.vehicle_capacity, new_util.customers)
        self.s = self.s_best
        self.n_iter = 0
    ...
    def step(self):
        if self.new_cycle == True:
            ...
            while(self.n_iter < self.max_iter):
                self.n_iter += 1
                new_s = util.generate_neighborhood(self.s, self.customers, self.vehicle_capacity)[random.randint(0,4)] #util.generate_initial_solution(15.5, customers)
                diff = util.cost_function(new_s, self.distances) - util.cost_function(self.s, self.distances)
                if (diff < 0):
                    self.s = new_s
                    self.new_cycle = True
                else:
                    prob = math.exp(-util.cost_function(self.s, self.distances)/self.t)
                    q = random.uniform(0,1)
                    if (q < prob):
                        self.s = new_s
                        self.new_cycle = True
                if (util.cost_function(self.s, self.distances) < util.cost_function(self.s_best, self.distances)):
                    self.s_best = self.s
